...
The `hidden` boolean attribute, when set (on any HTML element),
indicates that that element is not important for presentation of the
current state of the document. Other elements should generally not
reference hidden elements. The `hidden` attribute should not be used
to hide content visually that is otherwise relevant: if the document
could meaningfully be rendered a different way with that element
non-hidden, then the attribute is not appropriate.

### 6.2 Inert subtrees

Any node can be marked as inert, which makes it not responsive to text
selection, text search, and user interaction events. Modal dialogs
make the document "behind" them inert.

### 6.3 Activation

Some elements have an _activation behavior_, which is triggered when
the element is clicked, or receives a synthetic click, such as via the
`click()` method.
...
There can be a single _focused_ element in a document. This element
recieves all keyboard events generated by the user.

#### 6.4.2 Data model

A _focusable area_ is a region of the UI that can become
focused. Every focusable area has a _DOM anchor_, which is a node in
the DOM that represents the area (in many cases the area is itself a
node, and is its own anchor).

This section contains detailed algorithms for how focus is to be
implemented in user agents.

#### 6.4.3 The tabindex attribute

The `tabindex` attribute determines whether an element can be reached
via _sequential focus navigation_ (aka tabbing around a document). A
positive value gives the element's place in the navigation order; a
zero value just indicates that it is part of sequential focus
navigation, with no position requirement; a negative value excludes
the element from the navigation (but the element is still focusable).

#### 6.4.4 Processing model

Steps for user agents to implement when the focus changes.

#### 6.4.5 Sequential focus navigation

How user agents should implement sequential focus navigation.
...
/* partial */ trait Document {
  ...
  /** Returns the deepest element in the document through which or to
    * which key events are being routed.
    *
    * This is, roughly speaking, the focused element in the document.
    */
  def activeElement: Option[HTMLElement]

  /** Returns true if key events are being routed through or to the
    * document; otherwise, returns false.
    *
    * Roughly speaking, this corresponds to the document, or a
    * documented nested  inside this one, being focused.
    */
...
/* partial */ trait HTMLElement {
  ...
  /** Moves the focus to the element.
    *
    * If the element is a browsing context container, moves the focus
    * to the  nested browsing context instead.
    */
  def focus(): Unit

  /** Moves the focus to the viewport.
    *
    * Use of this method is discouraged; if you want to focus the
    * viewport, call the focus() method on the Document's root
    * element.
    *
    * Do not use this method to hide the focus ring if you find the
    * focus ring unsightly. Instead, use a CSS rule to override the
    * 'outline' property, and provide a different way to show what
    * element is focused. Be aware that if an alternative focusing
    * style isn't made available, the page will be significantly less
    * usable for people who primarily navigate pages using a keyboard,
    * or those with reduced vision who use focus outlines to help them
    * navigate the page.
    */
  def blur(): Unit
...
Describes using the `accesskey` attribute to assign a key combination
to a particular element to activate or focus it. This is not about
running a script when a particular key combination is pressed.
